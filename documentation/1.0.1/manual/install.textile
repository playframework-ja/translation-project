...
play のコマンドラインユーティリティは python を使用します。ですから、どのような UNIX システムでもそのまま使うことができます (ただし python 2.4 以上が必要です) 。 Windows で実行する場合でも、フレームワークに python ランタイムをバンドルしているので、心配する必要はありません。
