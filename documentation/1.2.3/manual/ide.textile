...
# IntelliJ IDEA の *File* メニューから *New Project…* を選択する
# *New Project* ウィザードで *Create project from scratch* を選択する
# *Next* ボタンをクリックする。
# ウィザードの次のページで *Project files location* を Play アプリケーションを含むディレクトリに変更する。
# ウィザードの同じページで *Project file format* を任意のフォーマットに設定する。
# ウィザードの同じページで *Create module* のチェックを外す (モジュールは既に存在しているため) 。
# *Finish* ボタンをクリックする。
# *File* メニューから *New Module…* を選択する
# *Add Module* ウィザードで *Import existing module* を選択し、アプリケーションディレクトリにある @.iml@ を選択する。
# *Finish* ボタンをクリックする。
