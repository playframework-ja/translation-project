...
Por defecto, el directorio @/public@ está vinculado a la URL @/public@, pero usted puede fácilmente cambiar esta configuración, o incluso utilizar diversos directorios para sus recursos estáticos.
...
Puede incluir archivos de configuración adicionales a la configuración de Play especificando un nombre de archivo en @application.conf@ como el valor de una opción de configuración que contiene <code>@include.</code> como principio de la clave. Por ejemplo, si define tipos MIME adicionales en un archivo @conf/mime-types.conf@
