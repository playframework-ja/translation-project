...
Play は @$PLAY_HOME/framework/lib/@ 内にH2とMySQLのJDBCドライバーを含みます。
PostgreSQL か Oracleデータベースを使いたければ、例えば、JDBCドライバーをそこに、または、アプリケーションの @lib/@ ディレクトリに追加します。
...
h3. <a name="multiple">複数データベースのサポート</a>

Playでは複数の (分離した) データベースを使うように設定することが可能です。

標準のデータベースは設定パラメーター @conf/application.conf@ の `db` で始めるキー (例: @db.url) で設定されます。追加データベースを設定する場合、以下のように `db` にアンダースコアとサフィックスを追加します。:
...
このデータベース設定は Play では 'other' と呼ばれます。例えば、この 'other' の JPA設定は以下のように設定します。: 

bc. db_other.jpa.dialect=<dialect>

アプリケーションの以下の Java コードからこの設定にアクセスできます。

bc. Connection conn = DB.getDBConfig("other").getConnection()

@DB.getDBConfig(configName)@ は @play.db.DB@ クラスでスタティックメソッドとして宣言されている全メソッドを持ったオブジェクトを返します。:
