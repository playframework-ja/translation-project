...
* モジュールの名前。 <code>==[a-zA-Z]+==</code> の正規表現にマッチしなければなりません。
* モジュールの短い概要
* プロジェクトホームページ
* OpenID
* モジュールは、ソースコードが利用可能で、かつバグを報告できるどこかで管理されていなければなりません。適当な場所を知らないのであれば、 <a href="http://github.com/">GitHub</a>, <a href="http://code.google.com/">Google Code</a> または <a href="http://launchpad.net/">Launchpad</a> が良い選択肢です。
