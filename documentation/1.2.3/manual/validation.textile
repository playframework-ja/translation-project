...
p(note). *制限事項*: この @validation.required(age)@ 構文を使った必須フィールドのバリデーションがひとつ以上失敗した場合、Play は正しいパラメータ名を判断することができます。この場合、フィールド名を直接、すなわち @validation.required("age", age)@ のように指定しなければなりません。
