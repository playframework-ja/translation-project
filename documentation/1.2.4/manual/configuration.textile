...
Configure su aplicación Play asignando valores a las claves de configuración del archivo @conf/application.conf@. Revise también:

* "Conceptos principales - el directorio conf":main#conf
* "Manejando application.conf en varios entornos":ids
...
La URL base de la aplicación usada para generar URLs absolutas. Esta es utilizada por la sintanxis de plantilla <code>@@{..}</code> y en los Jobs (los cuales no tienen un @Http.Request@ entrante), tales como la generación de e-mail. Por ejemplo, para el modo @dev@:
...
Permite que la sesión/cookie sea compartido entre subdominios. Por ejemplo, para hacer que los cookies sean válidos para todos los dominios que terminan en ‘.example.com’, tales como @foo.example.com@ y @bar.example.com@:

bc. application.defaultCookieDomain=.example.com

Por defecto: un cookie es válido solamente para un dominio específico.


h3(#application.lang.cookie). application.lang.cookie

Este será el nombre del cookie que se usa para almacenar el idioma actual, y normalmente lo asigna @play.i18n.Lang.change(String locale)@; usted puede cambiar este nombre si quiere configurariones de idiomas independientes para aplicaciones Play independientes. Por ejemplo:
...
Define las configuraciones regionales de su aplicación. Luego puede colocar los mensajes traducidos en los archivos @conf/messages.{idioma}@. El valor es una lista de códigos de idioma separados por coma, por ejemplo:

bc. application.langs=fr,en,ja

Por defecto: no tiene idiomas adicionales al original de la aplicación.
...
Ubicación de un archivo de configuración de Log4J, para personalizar la salida del resgistro de eventos (_log_). Si no especifica una ubicación, Play cargará el archivo @log4j.properties@ del directorio @conf@ en caso de que este archivo exista.
...
La clave secreta que se usa para proteger las funciones criptográficas, usualmente asignada por el comando @play new@ o @play secret@. Si usted despliega su aplicación en varias instancias asegurese de usar la misma clave. Por ejemplo:
...
Nombre del cookie de la sesión. Los cookies no están protegidos por defecto, configurelo solamente si de verdad está sirviendo sus páginas a través de HTTPS. Por ejemplo:

bc. application.session.cookie=PLAY

Por defecto: las sessiones se escriben en el cookie temporal @PLAY_SESSION@.


h3(#application.session.httpOnly). application.session.httpOnly

Habilita la bandera ‘HTTP only’ (_solamente HTTP_) en los cookies, la cual mitiga algunos ataques XSS. Por ejemplo:
...
Tiempo de expiración de la sesión, es decir, el tiempo máximo de vigencia del cookie de la sesión. Si no está configurado, la sesión expira cuando cierre su navegador web. Por ejemplo, para configurar el tiempo de la sesión en el valor una hora:

bc. application.session.maxAge=1h

Recordar la sesión por una semana:

bc. application.session.maxAge=7d

Por defecto: la sesión se basa en un cookie temporal que expira cuando el browser se cierra.


h3(#application.session.secure). application.session.secure

Habilita las sesiones basadas en cookie para conexiones HTTPS. Por ejemplo:
...
Evita el envio del cookie de la sesión si no hubo cambios en la misma. Por ejemplo:
...
Contraseña para un archivo de certificado X509 protegido por contraseña, para usar con la configuración "certificate.key.file":#certificate.key.file. Por ejemplo:
...
h2(#cron). Trabajos programados

Puede configurar expresiones _cron_ para los trabajos programados como claves de configuración con el prefijo @cron.@ y usar esas claves como el valor de una anotación <code>@play.jobs.On</code> o <code>@Every</code>. Por ejemplo, <code>@On("cron.noon")</code> se refiere a:
...
Establece el formato de fecha por defecto, usando un patrón de @java.text.SimpleDateFormat@. Por ejemplo:
...
Configuración del gestor de base de datos. Para configurar rápidamente una base de datos de desarrollo use una base de datos temporal en memoria (H2 en memoria):

bc. db=mem

Para una base de datos escrita en un simple archivo (H2 almacenada en archivo):
...
Un nombre genérico del método ‘destroy’. Cuando utiliza un _Datasource_ existente, a veces es necesario destruirlo cuando la aplicación es detenida. Por ejemplo:
...
Nivel de aislamiento de transacción de la base de datos. Por ejemplo:
...
La cantidad de segundos a esperar antes que las conexiones inactivas, que superan las "db.pool.minSize":#db.pool.minSize permitidas, sean ‘sacrificadas’. Para más información revise la "documentación de c3p0":http://www.mchange.com/projects/c3p0/#maxIdleTimeExcessConnections.
...
Una configuración completa del JDBC, va en conjunto con las propiedades "db.user":#db.user, "db.pass":#db.pass y "db.driver":#db.driver. Por ejemplo:
...
Dirección IP del oyente HTTP, para restringir las direcciones por la cuales el servidor va a escuchar las peticiones HTTP. Por ejemplo:
...
Depurar (_debug_) sentencias SQL (se registran en el log usando nivel DEBUG). Ejemplo:
...
Lista de nombres separados por coma de entidades JPA adicionales que se deben cargar. Esto es útil cuando tiene entidades adicionales que no están en el _package_ @models@ de la aplicación, tales como clases del modelo dentro de un JAR aparte. Ejemplo:
...
Define cual puerto utilizará JPDA cuando la aplicación esté en modo debug. Ejemplo:
...
Nombre de un algoritmo estándar del JDK Security API, para usar con la configuración "keystore.file":#keystore.file. Ejemplo:
...
Especifica un certificado de keystore, para soporte HTTPS. El archivo debe ser nombrado @certificate.jks@. Ejemplo:
...
Puede especificar multiples hosts para armar un cache distribuido. Por ejemplo:
...
Usted puede declarar tipos de archivo MIME adicionales. Por ejemplo:
...
Habilita el registro de eventos (_logging_) de las transacciones SMTP, Play usa JavaMail para realizar las transacciones SMTP reales.
...
Existen dos maneras de enviar el correo electrónico a través de un canal encriptado, con esta propiedad de configuración usted especifica cual de ellas va a utilizar. Valores disponibles:
...
* @want@ - el servidor _pedirá_ autenticación al cliente
* @need@ - el servidor _necesitará_ la autenticación del cliente
...
El límite en bytes al cual los archivos subidos serán escritos en el disco, por @org.apache.commons.io.output.DeferredFileOutputStream@. Por ejemplo:
...
h2(#xforwarded). Reenvíos del proxy
