...
Denominamos "binder":glossary#binder (vinculador) a una clase de Java que sabe cómo vincular un parámetro HTTP a un tipo de dato particular de Java.
...
h3. <a name="renderjson">Retornando una cadena de texto JSON</a>

Play provee una manera simple de retornar una cadena de texto JSON, utilizando los métodos @renderJSON(…)@. Estos métodos retornan una cadena de texto JSON y establecen el content type de la respuesta HTTP como @application/json@.

Puede especificar su propia cadena de texto JSON, o puede pasar cualquier dato de tipo @Object@, el cual será serializado por el @GsonBuilder@.  
...
En caso de tener un objeto cuya estructura sea más compleja, podrá utilizar @GsonBuilder@ para construir el JSON a retornar.
...
Si necesita más control a la hora de contruir la representación JSON de su objeto al pasar un @Object@ al método @renderJSON(…)@, puede pasar también serializadores GSON y @Type@ objects para personalizar la salida.

h3. <a name="renderxml">Retornando una cadena de texto XML</a>

Al igual que con los métodos JSON, hay varios métodos para retornar una cadena XML directamente desde el controlador. Los métodos @renderXml(…)@ retornan cadenas de texto XML con el contenido fijado como @text/xml@.  

En este caso podrá especificar su propia cadena XML, pasar un objeto de tipo @org.w3c.dom.Document@, o pasar un POJO que será serializado por el serializador XStream.
...
También puede pasar un objeto de tipo @org.w3c.dom.Document@.
