...
En este momento, no tenemos ninguna forma de crear nuevos posts o moderar los comentarios a través de la interfaz de usuario del blog. Play provee un módulo **CRUD**, listo para usar, que nos ayudará a generar un área de administración básica.

h2. <a>Habilitar el módulo CRUD</a>

Una aplicación Play puede ser ensamblada a partir de varios módulos de aplicación. Esto permite reusar componentes en diferentes aplicaciones o separar una aplicación grande en varias más pequeñas.
...
Para habilitar el módulo CRUD, agregue una línea en el archivo @/conf/dependencies.yml@, después de @require:@

bc. require:
    - play -> crud

Ahora ejecute el comando @play dependencies@ para resolver la nueva dependencia del módulo.

Este módulo viene con un conjunto genérico de **routes** que podemos reutilizar por ahora. Para importar estas **routes** solo hay que agregar esta línea al archivo @/yabe/conf/routes@:
