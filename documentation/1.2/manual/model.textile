...
インメモリの H2 データベースインスタンスを始めるには、単に **conf/application.conf** ファイルに次の一行を追加してください:
...
Play の配布物は **$PLAY_HOME/framework/lib/** ディレクトリに H2 と MySQL の JDBC ドライバを含みます。例えば PostgreSQL または Oracle データベースを使用する場合は、 **$PLAY_HOME/framework/lib/** または アプリケーションの **lib/** ディレクトリに JDBC ドライバを追加します。
