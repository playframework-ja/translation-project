...
コントローラ層は HTTP とドメインモデルの間の **インピーダンスミスマッチ** を減少させます。
...
SOAP のようないくつかの技術は Web を通してドメインモデルオブジェクトへのアクセスを提供します。しかし、SOAP はただの RPC スタイルプロトコルであり、この場合、HTTP はトランスポートプロトコルとして使用されます。アプリケーションプロトコルではありません。
...
独自の型をアプリケーションの @conf/application.conf@ ファイルに追加することもできます。例えば、 @.xcf@ 拡張子を持つ GIMP 画像向けの MIME タイプを追加するには、以下の行を追加します: 
...
@application.conf@ で @application.web_encoding@ を設定して、 Play がブラウザとコミュニケーションする際に使用するエンコーディングを指定します。この設定は、Play の強力な "Web サービスクライアント":libs#WebServiceclient のデフォルトエンコーディングも変更します。

bc. application.web_encoding=ISO-8859-1

@application.web_encoding@ の変更は @Content-type@ HTTP ヘッダの @charset@ の部分に影響します。動的な結果を転送する際に使用されるエンコーディングにも影響しますが、 Play が静的なコンテンツを提供する際のバイト転送には影響 **しません** : このため、デフォルトのレスポンスエンコーディングを変更しなければならず、 ( @public/@ フォルダの中に) 特別な文字を含む静的なテキストファイルがある場合、 これらのファイルが指定したエンコーディングに従って格納されていることを確認しなければなりません。その他のファイルは UTF-8 で保存されるべきです。
...
**@Catch** アノテーションで注釈されたメソッドは、別のアクションメソッドが特定の例外をスローした場合に実行されます。スローされた例外は @Catch メソッドの引数に渡されます。
...
**application.session.maxAge** プロパティが設定されていなければ、Web ブラウザを閉じたときにセッションは切れます。プロパティは以下のように設定します:

bc. application.session.maxAge=7d  # 1 週間有効にする
