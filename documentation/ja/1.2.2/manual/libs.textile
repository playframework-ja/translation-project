...
Promise は Play のいたるところ (Jobs や WS.async, その他…) で Future の代わりに使用されています。
...
サーバへ接続するためには、サービスプロバイダから得られる以下の情報を使って
OAuth.ServiceInfo インスタンスを作成する必要があります:
* リクエストトークン URL
* アクセストークン URL
* 認証 URL
* コンシューマ・キー
