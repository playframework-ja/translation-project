...
h2. <a>Then you install CrudSiena module</a>
...
* _@DateTime_ is managed and displays a timepicker but you need to add also the following annotation so that it gets the time:
bc. @As(lang={"*"}, value={"yyyy-MM-dd hh:mm:ss"})


p(note). Other annotations should be tested and implemented when missing
