...
It's a generic template engine that can be used by any Java applications to render any text type, html, xml, json, etc.  It also provides adapters specifically for the Play! Framework (please check out the other documentation for use with the Play Framework). 
...
1. Downloading from the Github repository. You can either checkout the entire project and find the distribution in the @dist@ directory, or you can click on the @download@ button and get the package in zip file. 
...
There are two classes that developers need to use: 
...
public class Foo {
 public String bar() {
     return JapidRender.render(arg1, arg2, etc);
 }
}
</code></pre>
...
h3. @cn.bran.japid.template.JapidTemplateBaseWithoutPlay@

This is the base class for all Java classes derived from the Japid templates. The template classes has one method named @render(arg1, arg2...)@ that developers can use to invoke them: 
...
The @RenderResult@ object contains the text which is the result of the template rendering and can be obtained with the @getText()@ method.
