...
p. The content negotiation module helps using content types which are not directly supported by the Play! framework.
...
p. The content negotiation module comes with several content types enabled by "default":http://github.com/oasits/play-content-negotiation/blob/master/conf/default . To make use of them you only
have to enable the module (see above).

h2. <a>Define a your own content type</a>

p. When you want to use your own content type you have to create a single properties file which holds the mapping
...
p. For example if you want to map the content type *application/vnd.com.example.play+xml* to the *.play* format you add the
following line

bc. application/vnd.com.example.play+xml=play

p. to an empty file. Save the file inside the */conf/* directory and name it *types*. Then reference it in your 
...
p. Besides defining your own content type and the corresponding template format, you may want at some point to actually use it.
