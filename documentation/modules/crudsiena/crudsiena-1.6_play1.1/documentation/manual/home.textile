...
h1. CRUD Siena Play! v1.6 release notes

* Play v1.2 support with 1.6_Play1.2 
* Play v1.1 support with 1.6_Play1.1 
* support for byte[]/enums/GAE blobs
...
h3. For Play! =1.2, install CrudSiena 1.6_play1.2:

bc.. play install crudsiena-1.6_play1.2

h3. For Play! =1.1.x_, install CrudSiena 1.6_play1.1:

bc.. play install crudsiena-1.6_play1.1
...
h2. <a>Temporary GAE Blob management in Siena/CrudSiena</a>

p(note). _After several requests about GAE Blob in Siena and as we are currently developing the new version of Siena which will manage byte[] directly and map them to Blob (JDBC/GAE/...) but it's not yet ready, I have decided to provide a workaround in CrudSiena until then._

* Temporary how-to for GAE Blob in Siena: 
** just use the GAE blob type
** add the @As(binder=models.crudsiena.GaeBlobBinder.class) to bind upload file to the GAE Blob.
** the binary data from the file are converted into a byte[] and encapsulated into the Blob. 
* **Warning**: _The mimetype of your file is basically lost during this operation as the Blob only keeps the bytes. That's why the Crud blobfield.html only displays the size of the byte[] and allows you to upload a new file._ 
