...
pre. <input name="user.name" value="${field.value}" id="YourID" class="class1 class2" required maxlength="8" pattern="[a-z]*>

p. *Note*: The @${field.value}@ expression will actually be resolved to the current value of the users name. @field.value@ is here used in conjunction with the "built-in field tag":http://www.playframework.org/documentation/1.1/tags#field but you can use other values like @${user?.name}@ too.
