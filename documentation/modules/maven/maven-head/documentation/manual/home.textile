...
プロジェクトを一から作成し始めるのであれば、 *play new myapp --with maven* コマンドでivyプロジェクトを作成することが出来ます。これによってモジュールが有効化され、 play mvn:init が実行されることになります。
...
play-parent プロジェクト（Maven pom プロジェクト）をローカルのMavenリポジトリにインストールします。その後適切な *pom.xml* を作成します。このステップの後、pom.xmlに依存関係を追記することができるようになります。
...
まず最初にapp/libフォルダをクリアした後、play mvn:upを実行します。

bc. play mvn:source // or play mvn:src

定義された依存関係を抽出して、もし存在すればソースコードを取得してapp/libフォルダにコピーします。
